...
.DESCRIPTION
    Connects to Microsoft Graph with delegated device code authentication and reports on:
      ...
      - Conditional Access–covered vs. per-user MFA state
...
function Connect-ToGraph {
    ...
    try {
        if ($TenantId) {
            ...
            Write-Host "[*] Using tenant-scoped device authentication through Microsoft Graph PowerShell." -ForegroundColor DarkCyan
        }
        ...
        Write-Host "[*] When prompted, open the Microsoft device login page and enter the displayed code." -ForegroundColor Yellow
...
function render(){
  ...
  document.getElementById('hero').innerHTML=[
    ...
    kpi(d.cntPhone,'Phone<br>SMS/Voice','legacy method',d.cntPhone>0?'warn':'neutral'),
  ].join('');
